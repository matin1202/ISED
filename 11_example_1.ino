...
#define _EMA_ALPHA 0.34    // EMA weight of new sample (range: 0 to 1)
                          // Setting EMA to 1 effectively disables EMA filter.
#define _K 10             // EMA repeat times.
...
unsigned long dist_prevs[10]; // unit: mm
int k = 1; // unit: times
...
void loop() {
  ...
  if(k < 10){
    dist_prevs[10-k] = dist_raw;
    k++;
    return;
  }

  dist_ema = EMA_calculate(0, dist_raw);
  for (int i = 9;i > 0;i--){
    dist_prevs[i] = dist_prevs[i-1];
  }
  dist_prevs[0] = dist_raw;
...
float EMA_calculate(int t, float dist){
  if (t < _K - 1){
    t++;
    return (1 - _EMA_ALPHA) * EMA_calculate(t, dist_prevs[t - 1]) + _EMA_ALPHA * dist;
  }
  return _EMA_ALPHA * dist;
}
