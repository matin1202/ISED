...
void loop() {
  ...
  else if(distance > _DIST_MIN && distance <= _DIST_CENTER){
    analogWrite(PIN_LED, 255.0 - 2.55 * (distance - 100.0));
  }
  else if(distance > _DIST_CENTER && distance < _DIST_MAX){
    analogWrite(PIN_LED, 2.55 * (distance - 200.0));
  }
  else{
